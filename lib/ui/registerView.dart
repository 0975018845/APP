...
import 'package:flutter_app2/button/GoogleSignInButton.dart';
...
class _RegisterViewState extends State<RegisterView> {
  ...
  Widget build(BuildContext context) {
        ...
        body : Container(
          decoration: BoxDecoration(
            image: DecorationImage(
              image: AssetImage('images/Teal Formal Medical Photo Organ Donation Poster (1).png'),
    ...
    child : MaterialButton(
    onPressed: () async {
    //bool ItSaved =  await SaveToFirebase(_emailField.text);
    bool shouldNavigate = await register(_emailField.text,_passwordField.text);
    if(shouldNavigate == true){
      Navigator.push(context,MaterialPageRoute(builder: (context) => MyHomePage(),
    ),
    );//navigate
      SaveToFirebase(_emailField.text,_userNameField.text);
    }},
    child: Text("Register"),
    textColor: Colors.white,
    ),
